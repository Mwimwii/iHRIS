...
$districts = I2CE_FormStorage::search('district');

foreach ($districts as $district_id) {
    if ( !($districtObj = $ff->createContainer('district|'.$district_id)) instanceof iHRIS_District ) {
        continue;
    }
    echo "Deleting district with id=" . 'district|'.$district_id . "\n";
    $districtObj->populate();
    $districtObj->delete(false,true);
}
