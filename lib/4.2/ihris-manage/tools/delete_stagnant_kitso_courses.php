...
$cadres = I2CE_FormStorage::search('cadre',false,$where);
print_r($cadres);

foreach(I2CE_FormStorage::listFields('cadre',array('id'), true) as $id => $data){
    $psObj = $ff->createContainer('cadre|'. $data['id']);
    $psObj->populate();
    $psObj->delete();
    $psObj->cleanup();
}
