...
class InfiniumAllRecordsProcessor extends Processor {
    ...
    function findPersonByOmangAndEmployeeNumber($create = false) {
        ...
        if ($id_type) {
            ...
            if (array_key_exists($id_num, $this->person_ids) === true) {
                $person_id = $this->person_ids[$id_num][$id_type];
            ...
            if (array_key_exists($id_num, $this->person_ids) === false) {
            ...
            $omang = strtoupper(trim($this->mapped_data['id_num']));
            'P' => trim($this->mapped_data['passport_num']);
            '1' => $omang_num;
            '4' => trim($this->mapped_data['employee_number']);
            '2' => $xcempt_num;
            );
    ...
    function loadPersonIds(){
        ...
        foreach($person_ids as $data){
            $personId[$data['id_num']][$data['id_type']] = $data['parent'];
          }
        return $personId;
      }
