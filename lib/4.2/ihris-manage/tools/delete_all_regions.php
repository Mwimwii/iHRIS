...
$regions = I2CE_FormStorage::search('region');

foreach ($regions as $region_id) {
    if ( !($regionObj = $ff->createContainer('region|'.$region_id)) instanceof iHRIS_Region ) {
        continue;
    }
    echo "Deleting region with id=" . 'region|'.$region_id . "\n";
    $regionObj->populate();
    $regionObj->delete(false,true);
}
