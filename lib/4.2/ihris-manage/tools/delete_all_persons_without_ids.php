...
foreach ( $persons as $person ) {
    ...
    // $person_ids = I2CE_FormStorage::search('person_id',true,$where);

    $person_ids = I2CE_FormStorage::listFields('person_id', array('id_type', 'id_num'), false, $where); 

    if (count($person_ids) == 0) {
        if ( ($personObj = $ff->createContainer('person|'.$person)) instanceof iHRIS_Person ) {   
            if ( prompt( "Should i delete person|$person", $do_delete ) ) {
                echo "Deleting person with id = ".$personObj->getFormID()."\n";
