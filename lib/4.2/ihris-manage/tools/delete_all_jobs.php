...
$jobs = I2CE_FormStorage::search('job');

foreach ($jobs as $job_id) {
    if ( !($jobObj = $ff->createContainer('job|'.$job_id)) instanceof iHRIS_Job ) {
        continue;
    }
    echo "Deleting job with id=" . 'job|'.$job_id . "\n";
    $jobObj->populate();
    $jobObj->delete(false,true);
}
