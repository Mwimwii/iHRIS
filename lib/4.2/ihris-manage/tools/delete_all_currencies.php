...
$currencies = I2CE_FormStorage::search('currency');

foreach ($currencies as $currency_id) {
    if ( !($currencyObj = $ff->createContainer('currency|'.$currency_id)) instanceof iHRIS_Currency ) {
        continue;
    }
    
    echo "Deleting currency with id=" . 'currency|'.$currency_id . "\n";
    $currencyObj->populate();
    $currencyObj->delete(false,true);
}
