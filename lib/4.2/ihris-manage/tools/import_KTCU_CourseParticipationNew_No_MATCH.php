...
class KTCU_CourseParticipationNew_No_MATCH extends Processor {
    ...
    protected function _processRow() {
        ...
        if ( ($personId = $this->findPersonByID(self::$omang_id_type,$this->mapped_data['omang'],$this->mapped_data['surname'],$this->mapped_data['firstname'],$this->mapped_data['middlename'])) === false) {
    ...
    function findScheduledCourse($course,$start,$end, $create =true) {
        ...
        if (!in_array($course,$this->course_ids)) {
            echo "course not found in training courses.\n";
            echo $course;
    ...
    function findPersonByID($id_type,$id_num, $surname, $firstname, $middle_name) {
        ...
        $personIDs = I2CE_FormStorage::listFields('person',array('id'),true,$where_names);
        if(empty($id_num)){
        if (count($personIDs) > 1) {
            $this->processStats('duplicate_id');
            echo "duplicate persons\n";
            //$this->duplicate_ids[] = $id_num;
            return false;
        } 
        else if (count($personIDs) == 0) {
            echo "creating a new person $firstname $surname\n";
          ...
          return $personID;
            ...
            echo "person found, now returning id ".$personObjId['id']."\n";
            return $personObjId;
          }
        }
        elseif(!empty($id_num)){
...
$processor = new KTCU_CourseParticipationNew_No_MATCH($file);
$processor->run();
