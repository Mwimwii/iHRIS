...
class InfiniumProcessor extends Processor {
    ...
    function getTransactionDate() {
        return $this->getDate($this->mapped_data['date']);
    }
    ...
    function processEmployeeExit() {
        ...
        if (! ($posObj = $this->ff->createContainer( $personPosObj->position))instanceof iHRIS_Position) {
            $this->addBadRecord("No position could be created");
            $personPosObj->cleanup();
            $personObj->cleanup();
            return false;
        }
    ...
    function addTermination() {
        ...
        if (!simple_prompt("Could not find position change reason $code: $title in " . implode(",",$existing_terminations) . "Should I add it?")) {
            return false;
        }
    ...
    function setNewPosition($personObj,$new_stat_code) {
        ...
        if ( ($facilityid = $this->lookupFacility( $this->mapped_data['new_unit_code'],$this->mapped_data['new_loc_code'])) ===false) {
            I2CE::raiseMessage("Could not find any facility [{$this->mapped_data['new_loc']}]associated to UnitCode({$this->mapped_data['new_unit_code']})  and LocCode({$this->mapped_data['new_loc_code']})");

            $this->addBadRecord("Facility not found is not in system");
            return false;
        }
        ...
        $newPersonPosObj->setParent($personObj);
    ...
    function lookupFacility($unit_code,$loc_code) {
        if (!array_key_exists($unit_code,$this->facLocations) || ! is_array($this->facLocations[$unit_code]) || !array_key_exists($loc_code,$this->facLocations[$unit_code]) 
            || !is_string($this->facLocations[$unit_code][$loc_code]) || !strlen($this->facLocations[$unit_code][$loc_code])) {
            return false;
        }
        return $this->facLocations[$unit_code][$loc_code];
    }
