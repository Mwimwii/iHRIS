...
class UpdateFromDuplicate extends Processor {
        ...
        //$person_id = false;
            ...
            elseif(array_key_exists($id_num, $this->person_ids) === false){ //search by names
                  ...
                  if (count($personIds) > 1) {
                    ...
                    foreach ($personIds as $key => $idvalue) {
                       $personObjdel = $this->ff->createContainer('person|'.$idvalue);
                        
                        echo "Deleting person with id = person|" . $idvalue . "\n";
                        $personObjdel->populate();
                        $personObjdel->populateChildren($child_forms);
                        foreach ($personObjdel->getChildren() as $child_form_name=>$child_form_data) {
                        ...
                        $personObjdel->delete(false,true);
                    }  
                  }
                  elseif(count($personIds == 1)){
                      $person = current($personIds);
...
$processor = new UpdateFromDuplicate($file);
$processor->run();
