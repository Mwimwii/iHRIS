...
$Course_ids = I2CE_FormStorage::search('training_course');
$sCourse_ids = I2CE_FormStorage::listFields('scheduled_training_course',array('training_course'));
foreach ($sCourse_ids as $sCourse_id=>$data) {
    $mapped = substr($data['training_course'],16);
    echo $mapped ."\n";
    if (in_array($mapped,$Course_ids)) {
        continue;
    }
    $where = array(
        ...
        'style'=>'equals',
        'data'=>array('value'=>'scheduled_training_course|' . $sCourse_id)
        );
    $psCourse_ids = I2CE_FormStorage::search('person_scheduled_training_course',false,$where);
    echo "\tdeleting " . count($psCourse_ids) . " references\n";
    foreach ($psCourse_ids as $psCourse_id) {
        $psObj = $ff->createContainer('person_scheduled_training_course|'. $psCourse_id);
        $psObj->populate();
        $psObj->delete();
        $psObj->cleanup();
    }
    $sObj = $ff->createContainer('scheduled_training_course|'. $sCourse_id);
    $sObj->populate();
    $sObj->delete();
    $sObj->cleanup();
...
foreach ($Course_ids as $Course_id) {
    ...
    $Obj = $ff->createContainer('training_course|'. $Course_id);
    $Obj->populate();
    $Obj->delete();
    $Obj->cleanup();
