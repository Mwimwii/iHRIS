...
class ImportPositionsByFacilityID extends Processor {
    ...
    protected function _processRow() {
        ...
        $facilityObj = $this->ff->createContainer('facility|'.$this->mapped_data['facility_id']);
        $facilityObj->populate();



        $jobObject = $this->createJob( trim($this->mapped_data['position']), trim($this->mapped_data['classification']), trim($this->mapped_data['salary_grade']) );
        $count = $this->mapped_data['establishment'];

        while($count > 0){    
            $positionObj = $this->createPosition($facilityObj, $jobObject, trim($this->mapped_data['position']), $this->mapped_data['position_type'],  trim($this->mapped_data['department']) );
            $count = $count - 1;
        }
    ...
    function creatCadre( $name ){
        $cadreObj = $this->listLookup( 'cadre', $name );
        $this->save($cadreObj);
    }

    function creatDepartment( $name ){
        $departmentObj = $this->listLookup( 'department', $name );
        $this->save($departmentObj);
    }
    ...
    function listLookup( $listform, $listValue, $otherFields=array() ){
      ...
      function createPosition( $facilityObj, $jobObj, $title, $position_type, $department ){
        ...
        $formObj->getField( 'pos_type' )->setFromDB( 'position_type|5' );
...
$processor = new ImportPositionsByFacilityID($file);
$processor->run();
