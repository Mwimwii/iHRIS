...
$facilities = I2CE_FormStorage::search('facility');

foreach ($facilities as $facility_id) {
    if($facility_id == 'facility|2425'){
        if ( !($facilityObj = $ff->createContainer('facility|'.$facility_id)) instanceof iHRIS_Facility ) {
            continue;
        }
        echo "Deleting facility with id=" . 'facility|'.$facility_id . "\n";
        $facilityObj->populate();
        $facilityObj->delete(false,true);
    }
