...
class InfiniumSnapshotResolver extends Processor {
    ...
    protected function getExpectedHeaders() {
        ...
        //    Unit,               Position, Title,               Grade, Authorized, Actual, Employee Name,        Employee Number
        //and a sample data row:
        //    Corporate Services, KA01AA,   Permanent Secretary, F1,    1,          1,      "SURNAME, FIRSTNAME", 359212208,       
        return  array(
            'org_level'=>'ORG. LVL',
            'unit_name'=>'Unit',
    ...
    protected function _processRow() {
        ...
        if (! ($personObj = $this->findPersonByEmployeeNumber(true)) instanceof iHRIS_Person) {
    ...
    protected static  $employee_number_id_type = 'id_type|4';
    protected static $excemption_id_type = 'id_type|2';
    protected $duplicate_ids = array();
    function findPersonByEmployeeNumber($create = false) {
        $personObj= false;
        ...
        $employee_number = strtoupper(trim($this->mapped_data['employee_number']));
        list($surname,$firstname) = explode(",",$this->mapped_data['name'],2);
        $surname = trim($surname);
        $firstname = trim($firstname);

        $id_type = false;
        if (strlen($employee_number) > 0 && ctype_digit($employee_number)) {
            $id_type = self::$employee_number_id_type;
        }else if (!in_array($employee_number, array('EXP', 'EXPATRIATE'))) {
        ...
        if ($id_type) {
            ...
            if (count($person_ids) > 1) {
                ...
                $this->duplicate_ids[] = $employee_number;
                return false;
            }
