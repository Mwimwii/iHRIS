...
class Cleanup_Import_Job_Final extends Processor {
    ...
    protected function _processRow() {
        ...
        if( empty($this->mapped_data['new_title']) && strtolower(trim($this->mapped_data['notes'])) == 'delete' ){
          $this->deleteJobPositions($this->mapped_data['original_title'], $this->mapped_data['new_title'], $this->mapped_data['notes']);
          return true;
        }
        ...
        $where_new = array(
          ...
          if(!empty($old_title)){
            ...
            if(count($oldjob_titles) >= 1){
                foreach($oldjob_titles as $id => $data){
                  ...
                  if( strtolower(trim($data['title'])) == strtolower(trim($new_title)) ){
                    ...
                    if(!empty($notes) && strtolower(trim($notes)) != 'delete'){
            ...
            elseif( count($oldjob_titles) == 0){
                $this->addBadRecord("old title Not found");
    ...
    function deleteJobPositions($old_title, $new_title, $notes){
          $where = array(
            'operator'=>'FIELD_LIMIT',
            'field'=>'title',
            'style'=>'lowerequals',
          ...
          $titles = I2CE_FormStorage::search('job', false, $where);
          if( count($titles) == 1){
              $jobObj = $this->ff->createContainer("job|".current($titles));
              $jobObj->populate();
              $jobObj->delete();
              $jobObj->cleanup();
            }
...
$processor = new Cleanup_Import_Job_Final($file);
$processor->run();
