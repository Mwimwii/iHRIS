...
class Cleanup_Import_Job extends Processor {
    ...
    protected function getExpectedHeaders() {
        return  array(
            ...
            'new_title' =>	'Agreed Upon Titles (Final)',
            'cadre' =>	'Occupational Group',
            'grouping' =>	'Grouping (Health professional, health associate professional, personal care worker, etc.)'
            );
    }
    ...
    protected function _processRow() {
        ...
        $this->jobTitleLookup($this->mapped_data['original_title'],
                              $this->mapped_data['new_title'],$this->mapped_data['cadre'],$this->mapped_data['grouping']);
        return true;
    }
    ...
    function jobTitleLookup( $old_title, $new_title, $cadre, $grouping ){
        if(!empty($new_title)){
          $where_old = array(
          ...
          $where_new = array(
            ...
            if(!empty($old_title)){
              $job_titles = I2CE_FormStorage::listFields('job', array('id', 'title'), false, $where_old); //search for the old title
              if(count($job_titles) >= 1){
                  foreach($job_titles as $id => $data){
              ...
              elseif( count($job_titles) == 0){
                  ...
                  if(count($new_job_titles) == 0){
                    ...
                    I2CE::raiseError("New title already exists");
                    }
                }
            }
          else{
    ...
    function checkPosition( $positionTitle, $newTitle ){
      if(!empty($newTitle)){
        ...
        if( count($titles) >=1 ){
            $this->addBadRecord("Found ".count($titles)." positions with old title");
...
$processor = new Cleanup_Import_Job($file);
$processor->run();
